...
function verifyLiveMetadata(target, outDir, metadataPath, metadata, metadataText, schemas, env) {
  ...
  if (metadata.adapter !== target.id)
    failures.push(`metadata adapter ${metadata.adapter} does not match registry id ${target.id}`);
  if (metadata.dryRun !== false) failures.push('metadata must come from a live run, not a dry run');
  if (metadata.status !== 'passed')
    failures.push(`metadata status must be passed, got ${metadata.status}`);
  if (metadata.exitCode !== 0)
    failures.push(`metadata exitCode must be 0, got ${metadata.exitCode}`);
  if ((metadata.missingEnv ?? []).length !== 0)
    failures.push(
      `metadata still reports missing runtime env: ${(metadata.missingEnv ?? []).join(', ')}`,
    );
  for (const name of target.requiredEnv) {
    if (!(metadata.requiredEnv ?? []).includes(name))
      failures.push(`metadata requiredEnv missing ${name}`);
  }
  if (metadata.artifactValidation?.ok !== true)
    failures.push('metadata artifactValidation.ok must be true');
  if (metadata.adapterConformance?.ok !== true)
    failures.push('metadata adapterConformance.ok must be true');
...
function verifyTarget(target, options, schemas) {
  ...
  const metadataSchemaFailures = validateSchema(
    metadata,
    schemas.externalRun,
    'guardbench-externalRun',
  );
...
export async function verifyExternalGuardBenchEvidence(options = {}) {
  ...
  const unknownAdapters = (options.adapters ?? []).filter(
  ...
  const schemaFailures = validateExternalEvidenceReport(report, {
    schema: options.evidenceSchema ?? DEFAULT_EVIDENCE_SCHEMA,
  });
  if (schemaFailures.length > 0) {
    throw new Error(
      `GuardBench external evidence schema validation failed: ${schemaFailures.join('; ')}`,
    );
  }
...
async function main() {
  ...
  if (args.json) {
    ...
    console.log(
      `GuardBench external evidence verification passed: ${verified} verified, ${pending} pending`,
    );
  } else {
