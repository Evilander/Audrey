...
export function formatImpactReport(report: ImpactReport): string {
  ...
  lines.push(
    `Memories: ${totalMemories} total (${report.totals.episodic} episodic, ${report.totals.semantic} semantic, ${report.totals.procedural} procedural)`,
  );
  lines.push(
    `Validated: ${report.validatedTotal} all-time, ${report.validatedInWindow} in last ${report.windowDays} days`,
  );
  const o = report.outcomeBreakdownInWindow;
  if (o.helpful + o.wrong + o.used > 0) {
    lines.push(
      `Outcomes (last ${report.windowDays} days): ${o.helpful} helpful, ${o.wrong} wrong, ${o.used} used`,
    );
  }
  lines.push('');
  ...
  lines.push('By type:');
  lines.push(
    `  Episodic:   ${report.byType.episodic.validated} validated, ${report.byType.episodic.recent} recent`,
  );
  lines.push(
    `  Semantic:   ${report.byType.semantic.validated} validated, ${report.byType.semantic.recent} recent, ${report.byType.semantic.challenged} challenges recorded`,
  );
  lines.push(
    `  Procedural: ${report.byType.procedural.validated} validated, ${report.byType.procedural.recent} recent`,
  );
  lines.push('');
