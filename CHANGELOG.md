...
- The vector index mirrors live rows only, and every writer agrees. Soft-forgetting a memory removed its vector while `memoryStatus()` kept counting the row, so the store reported itself unhealthy, and in strict mode Guard blocks every action on that report. A full re-embed, an import, or a resolve-truth supersede put vectors back for rows recall would never return. The health check now counts live rows; `audrey reembed`, import, and resolve-truth keep superseded rows out of the index; and a one-time repair on open drops the strays.
