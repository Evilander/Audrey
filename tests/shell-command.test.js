...
const READ_ONLY = [
  ...
  'env -C /tmp grep foo file',
  'env -i grep foo file',
  'env -u PATH grep foo file',
  'env --chdir=/tmp grep foo file',
  'nice -n 5 grep foo file',
  'nice -5 grep foo file',
...
const SIDE_EFFECT = [
  ...
  'printf -vfoo "%s" x',
  'history -w /tmp/h.txt',
  // Fourth and fifth review passes: a verb given by any path is never read-only.
  ...
  'env -S "rm -rf x"',
  'nice -n 5 rm x',
  '/usr/bin/../../../tmp/evil/grep x',
  '/usr/bin/../bin/grep x',
  'C:\\Program Files\\..\\..\\tmp\\evil\\grep.exe x',
  '/usr/bin//../tmp/evil/grep x',
  'jobs -x rm -rf /',
  'jobs -xl rm x',
  'bash -v',
...
describe('shell command signatures', () => {
  it.each([
    ...
    ['nice -n 10 npm run build', ['npm run build']],
    ['env -C /tmp rm -rf /', ['rm']],
    ['env -S "rm -rf x"', []],
