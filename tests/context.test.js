...
describe('contextMatchRatio', () => {
  ...
  it('divides by retrieval keys, not shared keys', () => {
    expect(contextMatchRatio({ task: 'debug' }, { task: 'debug', domain: 'payments' })).toBe(0.5);
  });

  it('returns 0 when shared keys all mismatch', () => {
    expect(contextMatchRatio({ task: 'debug' }, { task: 'deploy' })).toBe(0);
  });
});
...
describe('contextModifier', () => {
  ...
  it('returns 1.0 + weight when all keys match (default weight 0.3)', () => {
    expect(contextModifier({ task: 'debug' }, { task: 'debug' })).toBeCloseTo(1.3);
  });

  it('returns 1.0 when no keys match', () => {
    expect(contextModifier({ task: 'debug' }, { task: 'deploy' })).toBeCloseTo(1.0);
  });
  ...
  it('respects custom weight', () => {
    expect(contextModifier({ task: 'debug' }, { task: 'debug' }, 0.5)).toBeCloseTo(1.5);
  });
