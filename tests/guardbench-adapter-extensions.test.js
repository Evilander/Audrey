...
const summarySchema = JSON.parse(
  readFileSync('benchmarks/schemas/guardbench-summary.schema.json', 'utf-8'),
);
...
describe('GuardBench adapter extension evidence', () => {
  it('preserves unknown adapter fields under adapterExtensions', () => {
    const normalized = validateAdapterResult(
      adapterResult({
  ...
  it('rejects non-JSON extension values instead of serializing ambiguous evidence', () => {
    ...
    ).toThrow(/adapter extension probe must be JSON-serializable/);
    ...
    ).toThrow(/adapterExtensions must be a plain object when present/);
  });
