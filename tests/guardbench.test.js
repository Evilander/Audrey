...
} from '../benchmarks/guardbench.js';
import mem0Adapter, {
  createGuardBenchAdapter as createMem0GuardBenchAdapter,
} from '../benchmarks/adapters/mem0-platform.mjs';
import zepAdapter, {
  createGuardBenchAdapter as createZepGuardBenchAdapter,
} from '../benchmarks/adapters/zep-cloud.mjs';
import { writeGuardBenchConformanceCard } from '../benchmarks/create-conformance-card.mjs';
import {
  bundleRelativeFilePath,
  writeGuardBenchSubmissionBundle,
} from '../benchmarks/create-submission-bundle.mjs';
...
import {
  runGuardBenchAdapterSelfTest,
  validateAdapterSelfTestReport,
} from '../benchmarks/adapter-self-test.mjs';
import { validateAdapterSelfTestFile } from '../benchmarks/validate-adapter-self-test.mjs';
import {
  validatePublicationVerificationReport,
  verifyGuardBenchPublicationArtifacts,
} from '../benchmarks/verify-publication-artifacts.mjs';
import {
  buildExternalGuardBenchRun,
  evaluateAdapterConformance,
  parseExternalArgs,
} from '../benchmarks/run-external-guardbench.mjs';
import {
  buildExternalAdapterDryRunMatrix,
  validateExternalAdapterDryRunMatrix,
} from '../benchmarks/dry-run-external-adapters.mjs';
import {
  validateExternalEvidenceReport,
  verifyExternalGuardBenchEvidence,
} from '../benchmarks/verify-external-evidence.mjs';
import {
  computeGuardBenchArtifactHashes,
  validateGuardBenchArtifacts,
} from '../benchmarks/validate-guardbench-artifacts.mjs';
...
} from '../scripts/prepare-release-cut.mjs';
import {
  npmPackageTargetStatus,
  remoteBranchFreshnessStatus,
  targetChangelogStatus,
...
describe('GuardBench harness', () => {
  it('publishes scenario seeds and external adapter subjects in the manifest', () => {
    ...
    expect(
      manifest.subjects.some(subject => subject.name === 'Fixture Adapter' && subject.external),
    ).toBe(true);
    expect(manifest.scenarios).toHaveLength(10);
    expect(
      manifest.scenarios.every(scenario => scenario.seed && scenario.expectedEvidenceClass),
    ).toBe(true);
  ...
  it('validates the external adapter result contract directly', () => {
    ...
    expect(() =>
      ...
      /evidenceIds must contain only strings; summary must be a non-empty string; recallErrors must be an array when present/,
    );
  });
  ...
  it('exports a small adapter author kit for module and result validation', () => {
    ...
    expect(() => defineGuardBenchAdapter({ name: 'Missing Decide' })).toThrow(
      /must define async decide/,
    );
    expect(() =>
    ...
    ).toThrow(/decision must be one of allow, warn, block/);
  });
...
`,
  ...
  it('validates the GuardBench adapter registry', async () => {
    ...
    expect(validation.ok).toBe(true);
    expect(validation.adapters.map(row => row.id)).toEqual([
      'example-allow',
      'mem0-platform',
      'zep-cloud',
    ]);
  ...
  it('rejects missing external dry-run matrices in public GuardBench artifact sets', async () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'externalDryRun: Missing GuardBench external adapter dry-run matrix',
    );
    expect(validatePublicationVerificationReport(report)).toEqual([]);
  });
  ...
  it('rejects missing external evidence reports in public GuardBench artifact sets', async () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'externalEvidence: Missing GuardBench external evidence report',
    );
    expect(validatePublicationVerificationReport(report)).toEqual([]);
  });
  ...
  it('schema-validates publication verifier reports', async () => {
    ...
    expect(validatePublicationVerificationReport(malformed).join('\n')).toContain(
      'guardbench-publication-verification.checks: missing required property externalEvidence',
    );
  });
  ...
  it('verifies the browser launch plan against current publication copy', async () => {
    ...
    expect(
      report.targets.find(target => target.id === 'reddit-discussion').manualRuleCheckRequired,
    ).toBe(true);
    expect(report.targets.find(target => target.id === 'x-launch-thread').contentEntryIds).toEqual([
      'x-post-1',
      'x-post-2',
    ]);
  });
  ...
  it('strict browser launch results fail until post-submit URLs are recorded', async () => {
    ...
    expect(report.ready).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'strict launch readiness requires submitted targets',
    );
  });
  ...
  it('requires artifact URLs for submitted artifact-url launch targets', async () => {
    ...
    try {
      ...
      expect(report.ok).toBe(false);
      expect(report.failures.join('\n')).toContain(
        'x-launch-thread: submitted artifact-url target must record artifactUrl',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('records missing arXiv compile tooling as a pending machine-readable report', async () => {
    ...
    try {
      ...
      expect(verified.blockers.join('\n')).toContain(
        'Install tectonic, latexmk, or pdflatex+bibtex',
      );
  ...
  it('creates and verifies the paper submission bundle', async () => {
    ...
    try {
      ...
      expect(verified.files).toContain(
  ...
  it('rejects paper submission bundles when a listed file is modified after bundling', async () => {
    ...
    try {
      ...
      expect(verified.failures.join('\n')).toContain(
        'docs/paper/publication-pack.json: sha256 mismatch',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('reports 1.1 release readiness without hiding publish blockers', async () => {
    const report = await verifyReleaseReadiness({ targetVersion: '1.1.2', allowPending: true });
    ...
    expect(['pending', 'passed']).toContain(
      report.checks.find(check => check.id === 'npm-package-target').status,
    );
  ...
  it('keeps the 1.1 release cut idempotent after it is applied', () => {
    const report = prepareReleaseCut({ targetVersion: '1.1.2', date: '2026-07-13' });

    expect(report.ok).toBe(true);
    expect(report.apply).toBe(false);
    expect(report.currentVersions.packageJson).toBe('1.1.2');
...
`;
    ...
    expect(targetChangelogStatus(updated, '1.0.0')).toEqual({
      found: true,
      placeholderMarkers: [],
    });
  });
  ...
  it('keeps source-control readiness pending when live remote state cannot be verified', () => {
    const report = remoteBranchFreshnessStatus(
      { branch: 'master', upstream: 'origin/master', upstreamSha: 'abc1234' },
      () => ({
        status: 1,
  ...
  it('detects stale local upstream tracking refs before final release', () => {
    ...
    expect(report.blockers.join('\n')).toContain(
      'local origin/master is abc1234 but live remote is def9876',
    );
  });
  ...
  it('rejects placeholder release-cut changelog sections as final readiness evidence', () => {
    const status = targetChangelogStatus(
      `# Changelog
...
`,
  ...
  it('rejects malformed adapter registries', async () => {
    ...
    try {
      ...
      expect(validation.ok).toBe(false);
      expect(validation.failures.join('\n')).toContain(
        'Adapter missing-adapter path does not exist',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('rejects adapter registries with inconsistent metadata', async () => {
    ...
    try {
            ...
            adapters: [
              {
                ...
                commands: {
                  moduleValidate: 'npm run bench:guard:adapter-module:validate',
                  selfTest:
                    'npm run bench:guard:adapter-self-test -- --adapter benchmarks/adapters/example-allow.mjs',
                  selfTestValidate: 'npm run bench:guard:adapter-self-test:validate',
                  externalRun: 'npm run bench:guard:external',
                },
              },
      ...
      expect(validation.ok).toBe(false);
      expect(validation.failures.join('\n')).toContain(
        'credentialMode=none but declares requiredEnv',
      );
      expect(validation.failures.join('\n')).toContain(
        'command moduleValidate does not reference benchmarks/adapters/example-allow.mjs',
      );
      expect(validation.failures.join('\n')).toContain(
        'registry name Wrong Name does not match module name Example Allow Adapter',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
...
`,
  ...
  it('builds reproducible external GuardBench runs without embedding credentials', () => {
    const args = parseExternalArgs([
      ...
      '--check',
      '--out-dir',
      'benchmarks/output/external/mem0-test',
    ]);
  ...
  it('builds Zep external GuardBench runs without embedding credentials', () => {
    const args = parseExternalArgs([
      ...
      '--check',
      '--out-dir',
      'benchmarks/output/external/zep-test',
    ]);
    const run = buildExternalGuardBenchRun(args, {});
  ...
  it('builds a non-secret dry-run matrix for runtime external adapters', async () => {
    ...
    try {
      ...
      expect(matrix.adapters.find(row => row.id === 'mem0-platform').missingEnv).toEqual([
        'MEM0_API_KEY',
      ]);
      expect(matrix.adapters.find(row => row.id === 'zep-cloud').missingEnv).toEqual([
        'ZEP_API_KEY',
      ]);
  ...
  it('schema-validates external adapter dry-run matrices', async () => {
    ...
    try {
      ...
      expect(validateExternalAdapterDryRunMatrix(malformed).join('\n')).toContain(
        'guardbench-external-dry-run.adapters[0].status: expected one of dry-run-missing-env, dry-run-ready',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('reports pending external evidence separately from dry-run readiness', async () => {
    ...
    try {
      ...
      expect(strict.failures.join('\n')).toContain(
        'External evidence is pending for mem0-platform',
      );
  ...
  it('verifies live external evidence metadata without embedding runtime credentials', async () => {
    ...
    try {
      ...
      writeFileSync(
        join(outDir, 'external-run-metadata.json'),
        `${JSON.stringify(
          {
  ...
  it('rejects external evidence metadata that leaks runtime credential values', async () => {
    ...
    try {
      ...
      expect(report.failures.join('\n')).toContain(
        'metadata leaks runtime credential value for FIXTURE_API_KEY',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('rejects external adapter conformance when rows are missing', async () => {
    ...
    report.cases[0].results = report.cases[0].results.filter(
      row => row.system !== 'Incomplete Adapter',
    );
  ...
  it('rejects malformed published GuardBench artifact bundles', () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'guardbench-summary.rows[0].decision: expected one of allow, warn, block',
    );
  });
  ...
  it('rejects seeded raw-secret leaks in published GuardBench artifact bundles', () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'raw seeded secret leaked into GuardBench artifacts',
    );
  });
  ...
  it('rejects cross-artifact mismatches between summary, manifest, and raw output', () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'summary.cases vs raw.cases: cross-artifact mismatch',
    );
  });
  ...
  it('validates external-run metadata when a GuardBench output bundle includes it', () => {
    const report = withArtifactCopy(tempDir => {
      ...
      writeFileSync(
        join(tempDir, 'external-run-metadata.json'),
        `${JSON.stringify(
          {
            suite: 'GuardBench external adapter run',
            startedAt: '2026-05-13T00:00:00.000Z',
  ...
  it('rejects conformance cards when their artifact hashes do not match the bundle', () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'guardbench-conformance-card.json: integrity.artifactHashes.guardbench-raw.json does not match current artifact',
    );
  });
  ...
  it('records submission bundle files with portable relative paths on POSIX and Windows roots', () => {
    expect(
      bundleRelativeFilePath(
        '/tmp/audrey-bundle/root/schemas/schema.json',
        '/tmp/audrey-bundle/root',
      ),
  ...
  it('rejects submission bundles when the manifest violates the published schema', () => {
    ...
    try {
      ...
      expect(verification.ok).toBe(false);
      expect(verification.failures.join('\n')).toContain(
        'submission-manifest.json: submission-manifest: missing required property schemaVersion',
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('rejects external-run metadata when artifact hashes do not match the bundle', () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'external-run-metadata.json: artifactHashes.guardbench-summary.json does not match current artifact',
    );
  });
  ...
  it('rejects malformed external-run metadata in a GuardBench output bundle', () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.join('\n')).toContain(
      'guardbench-externalRun: missing required property validationCommand',
    );
  });
});
