...
`,
  ...
  it('reports 1.1 release readiness without hiding publish blockers', async () => {
    const report = await verifyReleaseReadiness({ targetVersion: '1.1.1', allowPending: true });
  ...
  it('keeps the 1.1 release cut idempotent after it is applied', () => {
    const report = prepareReleaseCut({ targetVersion: '1.1.1', date: '2026-07-10' });

    expect(report.ok).toBe(true);
    expect(report.apply).toBe(false);
    expect(report.currentVersions.packageJson).toBe('1.1.1');
